...
Results: Within each hospital, crude readmission risk varied widely across the twenty hospitals for AMI \Sexpr{readmission.props('ami')}, heart failure \Sexpr{readmission.props('heart_failure')}, and pneumonia \Sexpr{readmission.props('pneumonia')}. When crudely controlling for confounding, and compared to an arbitrary reference hospital, the odds ratio ranged from 0.95-1.02 for AMI, 0.92-1.04 for heart failure, and 0.96-1.04 for pneumonia. When we applied TMLE,
...
\subsection{Software}
The data were cleaned and prepared for statistical analysis using the Postgres relational database (version \Sexpr{system("psql --version | rev | cut -d' ' -f 1 | rev", intern=TRUE)}). We implemented our models using the R statistical package (version \Sexpr{getRversion()}),\supercite{team_r:_2014} using the "bigrf" package (version \Sexpr{packageVersion('bigrf')}) to grow the random forests.\supercite{lim_bigrf:_2014} We plotted our figures using the "ggplot2" package (version \Sexpr{packageVersion('ggplot2')}) .\supercite{wickham_ggplot2:_2009} All the code to develop used to process our data, fit our models, and typeset this article is \href{https://github.com/nograpes/tmle_readmissions}{available for download at Github}.
...
<<icd_codes, cache=FALSE, echo=FALSE, results='asis'>>=
suppressPackageStartupMessages(library(Hmisc))
icd.codes <- read.csv('~/repo/thesis/code/tmle/etude1_selection_icd.csv', stringsAsFactors=FALSE)
# Remove repition in ICD-codes when multiple descriptions.
icd.codes[c(FALSE,diff(as.numeric(gsub('V','',icd.codes[,1])))==0),1] <- ''
