...
Although the vote proportion is in the scale of zero to one, it is not calibrated well as a probability; to calibrate the vote proportion, we used Platt scaling (effectively logistic regression). 
...
We also estimated the effect of the twenty hospitals on the time-to-readmission. In general, we followed the method for (non-collaborative) TMLE for survival analysis outlined in Stitleman\supercite{}, with some deviation. 
...
for (disease in diseases){
  ...
  hazards <- ci.format(table[c('hazard.ratio','hazard.ratio.ci.low','hazard.ratio.ci.high')])
  
  tte <- do.call(paste0, list(round(table$mean.tte),' (',round(table$median.tte),')'))
  
  pre.format <- cbind(table[c('admitted','died','died.prop','live.discharge','readmitted','prop')],odds=odds,Q.star=table$Q.star,hazards=hazards,tte=tte)
...
#                      Q='Q',
#                      epsilon='ε',
                         Q.star='Marginal risk',
                         tte='Marginal mean TTE\n(median)'
                        ) [colnames(pre.format)]
  rownames(pre.format) <- NULL

  latex(pre.format, file='',
        col.just = rep('r',10),
        cgroup = c("", "30-day readmission", "Time-to-readmission"), 
        n.cgroup = c(6,2,2),
        rowlabel = "Hospital",
        caption.loc = "bottom",
        caption = pretty_names[disease])
}
@
...
% What about the whole repeated measure / hierarchical model thing?
