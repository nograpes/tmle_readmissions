...
% Instead of regressing every unit of time following discharge, we opted to preform a Cox proportional hazards model.

% Using the Cox proportional hazards model had the major advantage of not being computationally intense; a logistic regression regression would have required roughly 7 million data points (one for each day after discharge for each person) fit against thousands of variables. 




% Estimating the survivor function from the Cox proportional hazards function.
