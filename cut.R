...
load('df.object')
files=c('drugs.ahfs.object','drugs.object','diagnoses.object','procedures.object','just.hosps.object'
        ,'admit.diags.object')
...
save(disease.df,disease.big.matrix,file=paste(dump.dir,'heart.failure.object',sep='/'))

# Pneumonia
rows<-which(!is.na(df$admit_diag) & grepl('^48[0-7].*',df$admit_diag))
disease.df<-df[rows,]
disease.big.matrix<-cbind(big.matrix[rows,])
save(disease.df,disease.big.matrix,file=paste(dump.dir,'pneumonia.object',sep='/'))

# AMI
rows<-which(!is.na(df$admit_diag) & grepl('^410.*',df$admit_diag))
disease.df<-df[rows,]
disease.big.matrix<-cbind(big.matrix[rows,])
save(disease.df,disease.big.matrix,file=paste(dump.dir,'ami.object',sep='/'))
